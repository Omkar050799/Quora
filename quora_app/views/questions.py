...
from quora.permissions import is_super_admin_or_company_admin, is_super_admin_or_company_admin_or_employee
...
""" serializers """
from ..serializers.questions_serializer import QuestionsSerializer

''' swagger '''
from ..swagger.questions_swagger import (
...
class QuestionsView(CreateRetrieveUpdateViewSet, ApiResponse):
    ...
    model_class = Questions.objects.select_related("user")
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            # question payload
            question: str = req_data.get('question')

            required_list = {"question": question,}
            ...
            question_instance, error = create_or_update_serializer(QuestionsSerializer, req_data, sp1)
            if error:
                return ApiResponse.response_bad_request(self, message=error)

            req_data['id'] = question_instance.id
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        user_id: int = req_data.get('user')

        if user_id:
            req_data['user_id'] = user_id

        req_data['id'] = get_id
        question_instance, error = create_or_update_serializer(QuestionsSerializer, req_data, sp1, instance)
        if error:
            return ApiResponse.response_bad_request(self, message=error)
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = get_ordered_queryset(query_params, queryset, Questions)
    ...
    def transform_single(self, instance):
        resp_dict = {}
        resp_dict = Questions.to_dict(instance)
        return resp_dict
